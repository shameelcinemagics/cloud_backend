    ...
    try {
        ...
        if (role_slug) {
            ...
            const { data: role, error: roleError } = await supabaseAdmin
                ...
                .maybeSingle();
            if (roleError)
                return serverError(res, 'Failed to validate role');
            if (!role)
                return badRequest(res, `Role "${role_slug}" does not exist`);
            roleId = role.id;
        }
...
router.post('/set-user-page', requirePerm('settings', PERM.U), async (req, res) => {
    try {
        const { user_id, page_slug, level } = req.body;
        // Input validation
        ...
        if (pageErr)
            return serverError(res, pageErr.message);
        if (!page)
            return badRequest(res, 'Invalid page_slug');
        let perms_mask = null;
        if (level === 'view')
...
router.post('/assign-admin', requirePerm('settings', PERM.U), async (req, res) => {
    try {
        ...
        if (!role)
            return badRequest(res, 'Admin role missing. Please run migrations first.');
...
router.post('/set-role-pages', requirePerm('settings', PERM.U), async (req, res) => {
    try {
        const { role_slug, permissions } = req.body;
        // Input validation
        if (!isNonEmptyString(role_slug)) {
            return badRequest(res, 'Invalid role_slug');
        }
        ...
        const { data: pages, error: pagesError } = await supabaseAdmin
            ...
            .in('slug', pageSlugs);
        if (pagesError)
            return serverError(res, pagesError.message);
        if (!pages || pages.length === 0) {
...
router.post('/set-role-page', requirePerm('settings', PERM.U), async (req, res) => {
    try {
        const { role_slug, page_slug, level } = req.body;
        // Input validation
        if (!isNonEmptyString(role_slug)) {
            return badRequest(res, 'Invalid role_slug');
        }
        ...
        if (!page)
            return badRequest(res, `Page "${page_slug}" not found`);
        ...
        const { error } = await supabaseAdmin
            .from('role_page_perms')
            .upsert({ role_id: role.id, page_id: page.id, perms_mask }, { onConflict: 'role_id,page_id' });
        if (error)
            return serverError(res, error.message);
        return successResponse(res, { ok: true, level, perms_mask });
    }
...
router.get('/roles', requirePerm('settings', PERM.R), async (_req, res) => {
    try {
      ...
      `);
        if (permsError)
            return serverError(res, permsError.message);
...
router.post('/update-user-role', requirePerm('settings', PERM.U), async (req, res) => {
    try {
        const { user_id, role_slug } = req.body;
        // Input validation
        if (!isValidUUID(user_id)) {
            return badRequest(res, 'Invalid user_id format');
        }
...
// Delete a user
router.delete('/users/:user_id', requirePerm('settings', PERM.D), async (req, res) => {
    try {
        const { user_id } = req.params;
        if (!user_id || !isValidUUID(user_id)) {
...
router.put('/users/:user_id', requirePerm('settings', PERM.U), async (req, res) => {
    try {
        const { user_id } = req.params;
...
// Delete a role
router.delete('/roles/:role_id', requirePerm('settings', PERM.D), async (req, res) => {
    try {
...
// Update a role
router.put('/roles/:role_id', requirePerm('settings', PERM.U), async (req, res) => {
    try {
        const { role_id } = req.params;
        const { slug, label } = req.body;
        console.log('Update role request:', { role_id, slug, label });
        if (!role_id) {
            return badRequest(res, 'Invalid role ID');
        }
        const roleIdNum = parseInt(role_id, 10);
        ...
        if (checkError) {
            ...
            return serverError(res, 'Failed to check role existence');
        }
        if (!existingRole) {
            return badRequest(res, 'Role not found');
        }
