 ...
 */
export function parsePostgresError(error: PostgrestError): AppError {
  ...
  switch (pgCode) {
    ...
    default:
      return new AppError(
        ...
        500,
        { code: pgCode, originalError: message }
      );
  }
}
 ...
 */
export function formatErrorForLog(error: Error | AppError): Record<string, unknown> {
  const baseLog = {
    name: error.name,
 ...
 */
export function alreadyExistsError(resource: string): AppError {
  return new AppError(
 ...
 */
export function permissionError(message: string = 'Insufficient permissions'): AppError {
  return new AppError(
